  ...
  while (true)
    {
      ...
      rust_debug ("unify::go ltype={%s} rtype={%s}",
		  ltype->debug_str ().c_str (), rtype->debug_str ().c_str ());

      switch (ltype->get_kind ())
	{
	case TyTy::INFER:
	  return expect_inference_variable (static_cast<TyTy::InferType *> (
					      ltype),
					    rtype);
	...
	case TyTy::POINTER:
	  return expect_pointer (static_cast<TyTy::PointerType *> (ltype),
				 rtype);
	...
	case TyTy::PLACEHOLDER:
	  return expect_placeholder (static_cast<TyTy::PlaceholderType *> (
				       ltype),
				     rtype);
	...
	case TyTy::DYNAMIC:
	  return expect_dyn (static_cast<TyTy::DynamicObjectType *> (ltype),
			     rtype);

	case TyTy::CLOSURE:
	  return expect_closure (static_cast<TyTy::ClosureType *> (ltype),
				 rtype);
